using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using DataAccessLayer.Utility;
using System.Web.UI.WebControls;
using System.Data;


namespace DataAccessLayer.DTO.HumanResource
{
   public class A2ZDEDCTRLDTO
   {
       #region Propertise
       public int DedCode { set; get; }
       public string DedDesc { set; get; }
       public int DependsOn { set; get; }
       public Boolean Location { set; get; }
       public Boolean Percentage { set; get; }
       public Boolean ServType { set; get; }
       public Boolean Religion { set; get; }
      
      
    
      #endregion


       public static int InsertInformation(A2ZDEDCTRLDTO dto)
       {
           int rowEffect = 0;
           string strQuery = @"INSERT into A2ZDEDCTRL(DedCode,DedDesc,DependsOn,Location, Percentage,ServType,Religion) 
                     values('" + dto.DedCode + "','" + dto.DedDesc + "','" + dto.DependsOn + "','" + dto.Location + "','" +
                                 dto.Percentage + "','" + dto.ServType + "','" + dto.Religion + "')";
           rowEffect = Converter.GetInteger(BLL.CommonManager.Instance.ExecuteNonQuery(strQuery, "A2ZHRMCUS"));

           if (rowEffect == 0)
           {
               return 0;
           }
           else
           {
               return 1;
           }
       }
       public static A2ZDEDCTRLDTO GetInformation(int DedCode)
       {
           DataTable dt = BLL.CommonManager.Instance.GetDataTableByQuery("SELECT * FROM A2ZDEDCTRL WHERE DedCode = '" + DedCode + "'", "A2ZHRMCUS");


           var p = new A2ZDEDCTRLDTO();
           if (dt.Rows.Count > 0)
           {
               p.DedCode = Converter.GetInteger(dt.Rows[0]["DedCode"]);
               p.DedDesc = Converter.GetString(dt.Rows[0]["DedDesc"]);
               p.DependsOn = Converter.GetInteger(dt.Rows[0]["DependsOn"]);
               p.Location = Converter.GetBoolean(dt.Rows[0]["Location"]);
               p.Percentage = Converter.GetBoolean(dt.Rows[0]["Percentage"]);
               p.ServType = Converter.GetBoolean(dt.Rows[0]["ServType"]);
               p.Religion = Converter.GetBoolean(dt.Rows[0]["Religion"]);
               
               return p;
           }
           p.DedCode = 0;
           return p;
       }

       
       public static int UpdateInformation(A2ZDEDCTRLDTO dto)
       {

           int rowEffect = 0;
           string strQuery = "UPDATE A2ZDEDCTRL set dependsOn='" + dto.DependsOn + "', Location='" + dto.Location + "', Percentage='" + dto.Percentage + "', ServType='" + dto.ServType + "', Religion='" + dto.Religion + "' where DedCode='" + dto.DedCode + "'";
           rowEffect = Converter.GetInteger(BLL.CommonManager.Instance.ExecuteNonQuery(strQuery, "A2ZHRMCUS"));
           if (rowEffect == 0)
           {
               return 0;
           }
           else
           {
               return 1;
           }
       }
   }
}
